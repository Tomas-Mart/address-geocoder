package com.example.geocoder.integration;
...
class AddressGeocodingIntegrationTest {
    ...
    void shouldReturn201WhenBothApisWork() {
        ...
        // ✅ Dadata координаты должны быть всегда (основной API)
        assertThat(response.getBody().getDadataCoordinates()).isNotNull();
        // ✅ Yandex координаты могут быть null (вспомогательный API)
    ...
    @Test
    @DisplayName("POST - должен вернуть 422 когда оба API не нашли адрес")
    void shouldReturn422WhenBothApisFail() {
        ...
        assertThat(response.getBody()).contains("Не удалось получить координаты от Dadata");  // ← ИСПРАВЛЕНО
    }

    @Test
    @DisplayName("POST - должен вернуть 422 когда только Yandex нашел (Dadata = null, нельзя сохранить)")
    void shouldReturn422WhenOnlyYandexFound() {
        ...
        // Dadata = null → нельзя сохранить → 422
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.UNPROCESSABLE_ENTITY);
        assertThat(response.getBody()).contains("Не удалось получить координаты от Dadata");  // ← ИСПРАВЛЕНО
    }
    ...
    void shouldSaveResultToDatabase() {
        ...
        // ✅ Dadata координаты СОХРАНЯЮТСЯ
        assertThat(saved.getDadataCoordinates()).isNotNull();
        assertThat(saved.getDadataCoordinates().getLatitude()).isNotNull();
        assertThat(saved.getDadataCoordinates().getLongitude()).isNotNull();
        // ❌ Yandex координаты НЕ СОХРАНЯЮТСЯ
